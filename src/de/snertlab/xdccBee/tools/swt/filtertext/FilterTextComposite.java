...
public class FilterTextComposite extends Composite {
	...
	private void createClearTextNew(Composite parent) {
		...
		// natively
		if ((filterText.getStyle() & SWT.ICON_CANCEL) == 0) {
			final Image inactiveImage = new Image(this.getDisplay(),
					FilterTextComposite.class
							.getResourceAsStream(DISABLED_CLEAR_ICON));
			final Image activeImage = new Image(this.getDisplay(),
					FilterTextComposite.class.getResourceAsStream(CLEAR_ICON));
	...
	private void createSearchTextNew(Composite parent) {
		...
		if (true) {
			final Image activeImage = new Image(this.getDisplay(),
					FilterTextComposite.class.getResourceAsStream(SEARCH_ICON));
			final Label clearButton = new Label(parent, SWT.NONE);
			clearButton.setLayoutData(new GridData(SWT.BEGINNING, SWT.CENTER,
					false, false));
			clearButton.setImage(activeImage);
	...
	public void addFilterTextClearTextListener(
			IFilterTextClearTextListener filterTextClearTextListener) {
		listClearTextListener.add(filterTextClearTextListener);
	}

	public void removeFilterTextClearTextListener(
			IFilterTextClearTextListener filterTextClearTextListener) {
