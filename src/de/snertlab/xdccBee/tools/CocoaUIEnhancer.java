 ...
 * </p>
 * <p>
 * This code was influenced by the <a href="http://www.simidude.com/blog/2008/macify-a-swt-application-in-a-cross-platform-way/"
 * >CarbonUIEnhancer from Agynami</a> with the implementation being modified
 * from the <a href="http://dev.eclipse.org/viewcvs/index.cgi/org.eclipse.ui.cocoa/src/org/eclipse/ui/internal/cocoa/CocoaUIEnhancer.java"
...
static Callback proc3Args;
   ...
   *          The Display to use.
   * @param quitListener
   *          The listener to invoke when the Quit menu is invoked. This cannot
   *          be <code>null</code> or the SWT classes will complain.
   * @param aboutAction
  ...
  public void hookApplicationMenu(Display display, Listener quitListener,
    ...
    // This is our callbackObject whose 'actionProc' method will be called when
  ...
  private void initialize(Object callbackObject) throws Exception {
    ...
    // Create an SWT Callback object that will invoke the actionProc method of
    ...
    if (appName != null) {
      ...
      invoke(nsmenuitemCls, aboutMenuItem, "setTitle", new Object[] { nsStr }); //$NON-NLS-1$
      ...
      invoke(nsmenuitemCls, quitMenuItem, "setTitle", new Object[] { nsStr }); //$NON-NLS-1$
    }
   
   // Set MainMenu Title to appName
   Object nsStr = invoke(
            nsstringCls,
            "stringWith", new Object[] { appName //$NON-NLS-1$ //$NON-NLS-2$
            });
    ...
    nsStr = invoke(
            nsstringCls,
            "stringWith", new Object[] { "Dienste" //$NON-NLS-1$ //$NON-NLS-2$
            });
    Object prefServiceItem = invoke(nsmenuCls, appMenu, "itemAtIndex", //$NON-NLS-1$
        new Object[] { wrapPointer(kServicesMenuItem) });
    invoke(nsmenuitemCls, prefServiceItem, "setTitle", new Object[] { nsStr }); //$NON-NLS-1$
    invoke(nsmenuitemCls, prefServiceItem, "setEnabled", new Object[] { false }); //$NON-NLS-1$
    ...
    invoke(nsmenuitemCls, prefHideOthersItem, "setTitle", new Object[] { nsStr }); //$NON-NLS-1$
    ...
    invoke(nsmenuitemCls, prefShowAllItem, "setTitle", new Object[] { nsStr }); //$NON-NLS-1$
    ...
    // and we have registerd the new selectors on it. So just set the new action
    // to invoke the
    // selector.
