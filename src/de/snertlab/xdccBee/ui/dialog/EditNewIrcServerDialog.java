...
public class EditNewIrcServerDialog extends AbstractSaveDialog {
	...
	public EditNewIrcServerDialog(Shell parentShell, IrcServer ircServer, boolean isNew) {
	...
	protected String checkFields() {
		...
		if( StringUtils.isEmpty(txtHostname.getText()) ){
			msg = msg + XdccBeeMessages.getString("EditNewIrcServerDialog_CHECK_ERROR_HOSTNAME_EMPTY") + "\n";  //$NON-NLS-1$//$NON-NLS-2$
		}
		if( StringUtils.isEmpty(txtPort.getText()) ){
			msg = msg + XdccBeeMessages.getString("EditNewIrcServerDialog_CHECK_ERROR_PORT_EMPTY") + "\n";  //$NON-NLS-1$//$NON-NLS-2$
		}else if( ! StringUtils.isNumeric(txtPort.getText()) ){
			msg = msg + XdccBeeMessages.getString("EditNewIrcServerDialog_CHECK_ERROR_PORT_INTEGER") + "\n";  //$NON-NLS-1$//$NON-NLS-2$
		}
		return msg;
	...
	private void makeCompositeServerPreferences(Composite parent) {
		...
		lblHostname.setLayoutData( new GridData(SWT.RIGHT, SWT.CENTER, false, false) );
		lblHostname.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_HOSTNAME")); //$NON-NLS-1$
		
		txtHostname = new Text(comp, SWT.BORDER);
		txtHostname.setLayoutData( new GridData(SWT.FILL, SWT.CENTER, true, false) );
		
		Label lblPort = new Label(comp, SWT.NONE);
		lblPort.setLayoutData( new GridData(SWT.RIGHT, SWT.CENTER, false, false) );
		lblPort.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_PORT")); //$NON-NLS-1$
		...
		lblNickname.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_NICKNAME")); //$NON-NLS-1$
		...
		lblAutoconnect.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_AUTOCONNECT")); //$NON-NLS-1$
		...
		lblDebugMode.setLayoutData( new GridData(SWT.RIGHT, SWT.CENTER, false, false) );
		lblDebugMode.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_DEBUG_MODE")); //$NON-NLS-1$
		
		btnDebugMode = new Button(comp, SWT.CHECK);
		btnDebugMode.setLayoutData( new GridData(SWT.LEFT, SWT.CENTER, false, false, numCols-1, 0) );
		btnDebugMode.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_DEBUG_MODE_DESC")); //$NON-NLS-1$
	...
	private void makeCompositeChannels(Composite parent) {
		...
		lblTblChannels.setLayoutData( new GridData(SWT.LEFT, SWT.CENTER, false, false, numCols, 0) );
		lblTblChannels.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_LABEL_NAME_IRC_CHANNELS")); //$NON-NLS-1$

		Composite compTblChannel = new Composite(comp, SWT.NONE);
		compTblChannel.setLayout( new GridLayout() );
		compTblChannel.setLayoutData( new GridData(SWT.FILL, SWT.FILL, true, true) );

		tblViewerChannels = new TableViewer(compTblChannel, SWT.BORDER);
		Table tblChannels = tblViewerChannels.getTable();
		ContentLabelProviderTableIrcChannels contentLabelProviderTableIrcChannels = new ContentLabelProviderTableIrcChannels(tblChannels);
		tblViewerChannels.setContentProvider(contentLabelProviderTableIrcChannels);
		tblViewerChannels.setLabelProvider(contentLabelProviderTableIrcChannels);
		tblChannels.setHeaderVisible(true);
		tblChannels.setLinesVisible(true);
		GridData gridDataTblChannels = new GridData(SWT.FILL, SWT.FILL, true, true);
		...
		compChannelButtons.setLayoutData( new GridData(SWT.LEFT, SWT.FILL, false, true) );
		
		Button btnNewChannel = new Button(compChannelButtons, SWT.NONE);
		btnNewChannel.setLayoutData( new GridData(SWT.FILL, SWT.TOP, true, false) );
		btnNewChannel.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_BUTTON_NAME_NEW")); //$NON-NLS-1$
		...
		btnRemoveChannel.setLayoutData( new GridData(SWT.FILL, SWT.TOP, true, false) );
		btnRemoveChannel.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_BUTTON_NAME_REMOVE")); //$NON-NLS-1$
	...
	protected void myConfigureShell(Shell newShell) {
		if(isNew){
			newShell.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_TITLE_NEW")); //$NON-NLS-1$
		}else{
			newShell.setText(XdccBeeMessages.getString("EditNewIrcServerDialog_TITLE_EDIT")); //$NON-NLS-1$
		}
	}
	...
	private void callActionRemoveIrcChannel() {
		if(getSelectedIrcChannel() == null) return;
		new ActionRemoveIrcChannel(getSelectedIrcChannel()).run();
		fillTblChannels();
	}
	
	private IrcChannel getSelectedIrcChannel(){
		IStructuredSelection selection = (IStructuredSelection) tblViewerChannels.getSelection();
