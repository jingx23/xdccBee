...
public class QueueView extends Composite implements INotifyDccDownload {
	...
	private void createContentView() {
		...
		AutoResizeTableLayout autoTableLayout = new AutoResizeTableLayout(
				tblDownloadQueue);
		...
		column2.setText(XdccBeeMessages
				.getString("TABLE_DOWNLOADS_COL_PROGRESS")); //$NON-NLS-1$
		column3.setText(XdccBeeMessages
				.getString("TABLE_DOWNLOADS_COL_DOWNLOADRATE"));//$NON-NLS-1$
		column4.setText(XdccBeeMessages
				.getString("TABLE_DOWNLOADS_COL_DURATION"));//$NON-NLS-1$
		column5.setText(XdccBeeMessages.getString("TABLE_DOWNLOADS_COL_STATE"));//$NON-NLS-1$
		tblDownloadQueue.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true,
				true));
	...
	private void makeTableMenu() {
		...
		mntmAbortDownload.setText(XdccBeeMessages
				.getString("TABLE_DOWNLOADS_MNTM_ABORT_DOWNLOAD")); //$NON-NLS-1$
	...
	private DccDownload getSelectedDccDownload() {
		IStructuredSelection sel = (IStructuredSelection) tblViewerDownloadQueue
				.getSelection();
